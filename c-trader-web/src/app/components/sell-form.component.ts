...
@Component({
  ...
  template: `
    <form clrForm *ngIf="tickerData">
      ...
      <clr-input-container>
        <label>Volume ({{ quote_currenty }})</label>
        <input
          [(ngModel)]="volume"
          name="volume"
          clrInput
...
export class SellFormComponent implements OnInit {
  ...
  set bid(val: number) {
    if (this.tickerData)
      this._volume = this.toFixed(val * this.tickerData.a, 'price');
    this._bid = val;
  }
  get bid() {
    return this._bid;
  ...
  set volume(val: number) {
    if (this.tickerData)
      this._bid = this.toFixed(val / this.tickerData.a, 'quantity');
    this._volume = val;
  }
  get volume() {
    return this._volume;
  ...
  submit() {
    ...
    this.order.emit({
      price: Number.parseFloat(
        this.bid.toFixed(this.instrumentData.quantity_decimals)
      ),
    });
  }
